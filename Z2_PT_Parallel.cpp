    ...
    for (int j = 0; j < thermal_sweeps; j++) {
        #pragma omp parallel 
        {
            ...
            for (int i = 0; i < CONFIG; i++) {
                ...
                avg_plaq_array[i].value = update_heatbath(links_at_coupling[i], beta, rng_threads[tid]);
                }
            }
...
// ---------------------- Autocorrelation ----------------------
    ...
    for (int i = 0; i < CONFIG; i++) {
        ...
        for (int j = 0; j < autocorrelation_sweeps; j++) {
            double beta = beta_array[i];
            autocorrelation_array[j] = update_heatbath(links_at_coupling[i], beta, rng_threads[tid]);
        }
        ...
        #pragma omp single
            {
                ...
                for (int i=0; i<CONFIG; i++){
                    store_data[beta_index_array[i]]=avg_plaq_array[i].value;
                }
